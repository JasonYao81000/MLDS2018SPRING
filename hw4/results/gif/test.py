...
def test(agent, env, total_episodes=30):
    ...
    for i in range(total_episodes):
        ...
        if (len(rewards) == 0 or episode_reward > np.max(rewards)):
            print ('Saving frames to ep%03d_%03d.npy...'%(i, episode_reward))
            np.save('ep%03d_%03d.npy'%(i, episode_reward), np.array(frames))
