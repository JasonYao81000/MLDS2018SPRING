...
class Seq2Seq_Model():
    def __init__(self, rnn_size, num_layers, embedding_size, 
                    optimizer, learning_rate, word_to_idx, mode, max_gradient_norm, 
                    use_attention, use_scheduled, sampling_probability, beam_search, beam_size, 
    ...
    def build_model(self):
        ...
        with tf.variable_scope('encoder', reuse=tf.AUTO_REUSE):
            ...
            # Build embedding matrix, encoder and decoder using together.
            w2v_model = Word2Vec.load('./w2vModel_256_w5_mc1_iter300.bin')
            w2v_embedding = np.zeros([len(w2v_model.wv.index2word), self.rnn_size])
            for index, word in enumerate(w2v_model.wv.index2word):
                w2v_embedding[index, :] = w2v_model[word]
            embedding = tf.Variable(tf.convert_to_tensor(w2v_embedding, np.float32), trainable=False, name='embedding')
        ...
        with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
            ...
            if self.optimizer == 'sgd':
                ...
                decayed_learning_rate = tf.train.exponential_decay(self.learning_rate,
